...
prompt_ξ = tk.Label(root, text='Коэффициент связности — ξ')
prompt_ξ.grid(row=3, column=0, sticky='w')
field_ξ = tk.Entry(root)
field_ξ.grid(row=3, column=1)
field_ξ.insert('end', 0.35)

# Поле для ввода времени выполнения операций
prompt_TO = tk.Label(root, text='Время выполнения операций — TO')
prompt_TO.grid(row=4, column=0, sticky='w')
...
prompt_τ = tk.Label(root, text='Время цикла оперативной памяти — τ')
prompt_τ.grid(row=5, column=0, sticky='w')
...
horiz_scroll.grid(row=1, column=0, sticky='NSEW')
vert_scroll = tk.Scrollbar(equations_frame, orient='vertical')
vert_scroll.grid(row=0, column=1, sticky='NSEW')
