...
def probability_specific(j, l, m): # Вероятность нахождения системы в состоянии j
    ...
    return (math.factorial(l) / custom_prod(j) * custom_prod(m, j, ρ)) /\
        recursive_sum(0, j[:]) # Передать копию списка, чтобы не модифицировать исходный
...
def plot_graph_for(configuration, index, up_to): #index — для процессоров какого типа строить график
    ...
    for m[index] in range(0, up_to + 1):
        ...
        queue.append(mean_queue_lenght(m))
    ...
    # Вывод статистики
    print('{:<4} {:<20} {:<20}'.format(f'm-{index}', 'Производительность', 'Ср. Длина очереди'))
    for m_index_values, perf, queue in zip(m_index_values, perf, queue):
        print('{:<4} {:<20} {:<20}'.format(m_index_values, perf, queue))
